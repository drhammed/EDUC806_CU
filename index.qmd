...
![](images/Levene_Test-Homogeneity%20of%20Variance.png){fig-align="center"}
...
![](images/mean%20RT%20per%20person.png){fig-align="center"}
...
![](images/within-subject%20effects.png){fig-align="center"}
...
In the last tutorial, we talked about one-way ANOVA which included one factor (explanatory variable). But imagine we have more than one factors (which of course would be close to reality in research). Then in this case, we can perform a **factorial** ANOVA.
...
-   $\alpha_i =$ the effect of level $i$ of factor A. In other words, the "**main effects"** of factor A

-   $\beta_j =$ the effect of level $j$ of factor B. In other words, the "**main effects"** of factor B

-   $(\alpha \beta)_{ij} =$ the "**interaction term**" between the two main effects (a single parameter).

-   $\epsilon_{ijk}$ are random normally $N(0, \alpha^2)$ distributed variables.

-   $i$ = 1,2,...$I$, ($I$ is the number of levels of factor A).

-   $j$ = 1,2,...$J$ ($J$ is the number of levels of factor B).

-   $k$ = 1,2,...$K$ ($K$ is the number of replicates per group).

In this kind of model, we can consider a group as a combination of a level from each of the factors. Such that for example, $y_{248}$, would represent the measurement of the 8th replicate of the group for level 2 of factor A and level 4 of factor B. This kind of model is called $I \times J$ "Factorial design" with $K$ replicates.

Therefore, a two-way factorial ANOVA is a statistical analysis that is used to examine the impact of two independent categorical variables on a dependent variable. We used this to test the "main effects" of each independent (factor) variable, as well as determine if there is any "interaction" between the independent variables. Specifically, we can examine if the effect of one independent variable on the dependent variable is consistent across all levels of the other independent variable.

#### *Let's explain this with data and do this in SPSS*

The data for this tutorial is based on the study of Maglio and Polman (2014). This is an open-data and available on OSL open stats lab (thanks to the folks doing this). At the end of this tutorial, you should be able to reproduce the analysis and [Figure 1](https://journals.sagepub.com/stoken/default+domain/hv65WpzIjfXtVjXCbJw6/full) of this paper.

**Brief study description**

"A person's perceived distance from a location can be affected by the actual spatial distance to that

location, but by psychological factors as well. For example, the distance to the nearest gas station may

seem farther when one's car is running low on gas. In their research, Maglio and Polman (2014) sought

to examine whether a person's spatial orientation (facing toward a location vs. facing away from it)

would also affect its perceived distance."

"To study this, the researchers recruited 202 riders on the Toronto subway green line. All participants were recruited at the Bay subway station (see map below). Half of the participants were headed eastbound (toward Bloor-Yonge and Sherbourne) whereas the other half were headed westbound (toward St. George and Spadina). Participants were randomly assigned to indicate the subjective distance of one of the four subway stations (Bloor-Yonge, Sherbourne, St. George, or Spadina)".

![](images/subway_TO.png){fig-align="center"}

**Things to do-**

1.  Open the data file (called Maglio and Polman Experiment 1). Explore the data file. This is the [link](https://drive.google.com/drive/folders/1I17EmHGdDHAmqOjEZ5X7tnM3DpXHUD4I?usp=sharing).
...
4.  Perform independent samples t-tests to determine if orientation is significantly different at each station.
5.  Generate a line graph to depict the interaction between orientation and station.
6.  Discuss the results of the analyses conducted above.
...
To start with, we will check assumptions throughout the analysis.

Notice that the sample sizes are unequal. I mean, the number of participants within each level combination of factors (i.e., Spadina East, Spadina West, etc.) varies. Four cells have 26 participants, three cells have 25 participants, and 1 cell has 23 participants.

You can copy and paste this Syntax to your SPSS and get the summary stat for everything

```{DATASET ACTIVATE  maglio.}
AGGREGATE
    /OUTFILE=*MODE=ADDVARIABLES
    /BREAK direction station
    /mean = MEAN(subjective_distance)
    /count = N.
DATASET NAME  maglio_summary.
```

It is important to note that Uneven sample sizes creates an **unbalanced design** and this will affect the analysis.

First, we should recode the "direction" so we can use it in subsequent analysis. Copy and paste the syntax below to your SPSS and run it

```{*Recode the direction (east = 0, west =1 )}

RECODE direction ('EAST'=0) ('WEST'=1) INTO direction_recode.
EXECUTE.
...
A factorial ANOVA compares the means across two or more explanatory variables, thereby splitting the sample into four or more groups (*remember a one-way ANOVA only has one independent variable that splits the sample into two or more groups)*.

In terms of analyzing variance, if we have more than one independent variable, then you can conduct a factorial ANOVA. Notice that, in both cases, we only have *one* dependent variable.

The presence of two independent variables classifies a factorial ANOVA as two-way. The assumptions of a two-way ANOVA echo those of the one-way ANOVA (we discussed last week).

Three are met before data collection, three are tested.
...
5.  **Normality**, assessed with the Shapiro-Wilk test for each level combination of the two factors;
...
The null and alternative hypotheses, assessed with the *F* test, can make claims about the means between cells or claims about the interaction between factors.

#### Two-Way ANOVA

Since we have an unbalanced design with unequal sample sizes in each cell, we must specify the type of *sum of squares*. Type-III is usually [recommended](http://www.sthda.com/english/wiki/two-way-anova-test-in-r) in this case. When the design in unbalanced, each type of *sum of squares* will yield different results.

With that in mind, let's start by constructing the ANOVA.

Copy and paste this into your syntax

```{*ANOVA.}

DATASET ACTIVATE DataSet1.
UNIANOVA subjective_distance BY orientation station
  /METHOD=SSTYPE(3)
  /INTERCEPT=INCLUDE
  /PLOT=PROFILE(station*orientation) TYPE=LINE ERRORBAR=NO MEANREFERENCE=NO YAXIS=AUTO
  /CRITERIA=ALPHA(0.05)
  /DESIGN=orientation station orientation*station
  /SAVE RESID.
```

Also, we can summarize the model to test for interaction effects. This include adding the line "print ETASQ" to the above code
...
![](images/ANOVA_1.PNG){fig-align="center"}

From the result we can see there are two significant effects, one on station and the other on the **interaction** between orientation and station.

**Interpretation**

The two-way ANOVA revealed no main effect on orientaion, *F* \< 1, *p* = .574. The *F*-value did not exceed the critical value. However, there was a significant effect on station, *F*(3, 194) = 24.100, *p* \< .001. The interaction between orientation and station was also significant, *F*(3, 194) = 16.283, *p* \< .001.

**Outliers**

Outliers can reduce the accuracy of results and skew data. Copy and paste the code below to your SPSS

```{*check for outliers}

EXAMINE VARIABLES=subjective_distance BY station BY orientation
  /PLOT=BOXPLOT
  /STATISTICS=NONE
  /NOTOTAL.
...
![](images/outliers_twowayANOVA.PNG){fig-align="center"}
...
![](images/normality_twowayANOVA.PNG){fig-align="center"}

Since *p* \< .001, we reject the null hypothesis: ***the data are not normally distributed***. The ANOVA is a robust procedure, so it can withstand violations of normality (or you can decide to consider the equivalent non-parametric test).

**Homoscedasticity**

Recal that the null hypothesis of Levene's test is that the variances are homogenous.
...
![](images/Homoge_twowayANOVA.PNG){fig-align="center"}

Looking at the result and since *p* = 0.773 (\> 0.05), there is no evidence in favor of rejecting the null hypothesis. **The data are homoscedastic**.
...
Social scientists usually report Cohen's *d* as the effect size of independent t-tests (we shall come back to this later in the semester more extensively). Cohen's *d* is standardized. However, in the present study, the author's chose to report partial eta-squared, which indicates the [proportion of overlap](http://psychology.okstate.edu/faculty/jgrice/psyc3214/independentsamples_t_test.pdf) between the factor and the reponse variable.

Generally, effect sizes are considered:
...
Copy and paste this chunk to your SPSS and run them. First, we need to split the dataset by station. Here, we want to split the "spadina" station (West and East). Run the "filter spadina west" chunk. Do the same for "spadina east". Then merge them together and finally compute the T-test on this data.

```{*filter spadina west}

DATASET ACTIVATE DataSet1.
DATASET COPY  spad_west.
DATASET ACTIVATE  spad_west.
FILTER OFF.
USE ALL.
SELECT IF (station = 1 & direction = "WEST").
EXECUTE.
DATASET ACTIVATE  DataSet1.
...
DATASET ACTIVATE DataSet1.
DATASET COPY  spad_east.
DATASET ACTIVATE  spad_east.
FILTER OFF.
USE ALL.
SELECT IF (station = 1 & direction = "EAST").
EXECUTE.
DATASET ACTIVATE  DataSet1.
...
DATASET ACTIVATE spad_east.
SORT CASES BY direction orientation station subjective_distance direction_recode RES_1.
DATASET ACTIVATE spad_west.
SORT CASES BY direction orientation station subjective_distance direction_recode RES_1.
DATASET ACTIVATE spad_east.
MATCH FILES /FILE=*
  /FILE='spad_west'
  /BY direction orientation station subjective_distance direction_recode RES_1.
EXECUTE.



*compute t-test for spadina

DATASET ACTIVATE spad_east.
T-TEST GROUPS=direction_recode (0 1)
  /MISSING=ANALYSIS
  /VARIABLES=subjective_distance
  /ES DISPLAY(TRUE)
  /CRITERIA=CI(.95).
...
![](images/t_test-spadina.PNG){fig-align="center"}

***Westbound participants rated Spadina station as being closer than eastbound participants, p \< .001.***

Then we need to check for all the assumptions
...
DATASET ACTIVATE spad_east.
EXAMINE VARIABLES=subjective_distance BY direction
  /PLOT=BOXPLOT
  /STATISTICS=NONE
  /NOTOTAL.
```

![](images/spadina_outliers.PNG){fig-align="center"}
...
![](images/spadina_normality.PNG){fig-align="center"}

Both Sig. \< .05, so we reject the null hypothesis: the data are not normal.

**Homoscedasticity**

```{*Homoscedasticity}

ONEWAY subjective_distance BY direction_recode
  /ES=OVERALL
...
![](images/spadina_homoge.PNG){fig-align="center"}

The variances are homogenous, *p* = 0.701.

**Now, we will do the same for all other stations**

**St-George**

```{*filter st_george west}

DATASET ACTIVATE DataSet1.
DATASET COPY  st_george_west.
DATASET ACTIVATE  st_george_west.
FILTER OFF.
USE ALL.
SELECT IF (station = 2 & direction = "WEST").
EXECUTE.
DATASET ACTIVATE  DataSet1.
...
DATASET ACTIVATE DataSet1.
DATASET COPY  st_george_east.
DATASET ACTIVATE  st_george_east.
FILTER OFF.
USE ALL.
SELECT IF (station = 2 & direction = "EAST").
EXECUTE.
DATASET ACTIVATE  DataSet1.
...
![](images/t_test-st_george.PNG){fig-align="center"}

***Westbound participants rated St. George as being closer than eastbound participants, p \< .001.***

**Check for assumptions- Outliers, normality and homogeneity of variance**
...
![](images/st_george_outliers.PNG){fig-align="center"}
...
![](images/st_george_normality.PNG){fig-align="center"}

Since both Sig. \< .05, we accept the alternative hypothesis: the data are not normally distributed.
...
![](images/st_george_homoge.PNG){fig-align="center"}

Since *p* = .666, we accept the null hypothesis: the data are homoscedastic.

**Bloor_yonge Station**

```{*filter bloor_yonge_west}

DATASET ACTIVATE DataSet1.
DATASET COPY  bloor_yonge_west.
DATASET ACTIVATE  bloor_yonge_west.
FILTER OFF.
USE ALL.
SELECT IF (station = 3 & direction = "WEST").
EXECUTE.
DATASET ACTIVATE  DataSet1.
...
DATASET ACTIVATE DataSet1.
DATASET COPY  bloor_yonge_east.
DATASET ACTIVATE  bloor_yonge_east.
FILTER OFF.
USE ALL.
SELECT IF (station = 3 & direction = "EAST").
EXECUTE.
DATASET ACTIVATE  DataSet1.
...
DATASET ACTIVATE bloor_yonge_east.
MATCH FILES /FILE=*
  /FILE='bloor_yonge_west'
  /BY direction orientation station subjective_distance direction_recode RES_1.
EXECUTE.



*compute t-test for bloor_yonge
...
![](images/t_test-bloor-yonge.PNG){fig-align="center"}
...
![](images/bloor-yonge_outliers.PNG){fig-align="center"}
...
![](images/bloor-yonge_normality.PNG){fig-align="center"}

Both Sig \< .05, so we reject the null hypothesis. The normality assumption is violated.
...
![](images/bloor-yonge_homoge.PNG){fig-align="center"}

*P* = 0.061, we accept the null hypothesis: the variances are homogenous.

Finally, we can compute the test for the 4th train station

**Sherbourne**

```{*filter sherbourne_west}

DATASET ACTIVATE DataSet1.
DATASET COPY  sherbourne_west.
DATASET ACTIVATE  sherbourne_west.
FILTER OFF.
USE ALL.
SELECT IF (station = 4 & direction = "WEST").
EXECUTE.
DATASET ACTIVATE  DataSet1.
...
DATASET ACTIVATE Dataset1.
DATASET COPY  sherbourne_east.
DATASET ACTIVATE  sherbourne_east.
FILTER OFF.
USE ALL.
SELECT IF (station = 4 & direction = "EAST").
EXECUTE.
DATASET ACTIVATE  DataSet1.
...
DATASET ACTIVATE sherbourne_east.
MATCH FILES /FILE=*
  /FILE='sherbourne_west'
  /BY direction orientation station subjective_distance direction_recode RES_1.
EXECUTE.



*compute t-test for sherbourne
...
![](images/t_test-sherbourne.PNG){fig-align="center"}
...
![](images/sherbourne_outliers.PNG){fig-align="center"}
...
![](images/sherbourne_normality.PNG){fig-align="center"}

**The data are not normally distributed (both Sig. \< 0.05).**
...
![](images/sherbourne_homoge.PNG){fig-align="center"}

The variances are equal (*p* = 0.657).

Finally, let's visualize the data to show the relationship between station and the direction of travel (orientation)- this is similar to the one in [Figure 1](https://journals.sagepub.com/stoken/default+domain/hv65WpzIjfXtVjXCbJw6/full) of this paper.

```{DATASET ACTIVATE DataSet1.}
GRAPH
  /LINE(MULTIPLE)=MEAN(subjective_distance) BY station BY direction.

```

![](images/last_line_graph_2.PNG){fig-align="center"}

N.B.- For good-publication friendly- plots, I strongly recommend using R/R-Studio. See the same plot in R- which clearly looks more like the one in the paper- including the CI)

![](images/Fig_1_R-plot.png){fig-align="center"}

**Conclusions**

Based onn the result, we can conclude that the two-way ANOVA revealed no main effect on orientaion, *F* \< 1, *p* = .574. The *F*-value did not exceed the critical value. However, there was a significant effect on station, *F*(3, 194) = 24.100, *p* \< .001. The interaction between orientation and station was also significant, *F*(3, 194) = 16.283, *p* \< .001. Investigating this interaction revealed that eastbound participants perceived stations to the East as being closer than did westbound participants, both for Bloor-Yonge (*p* = .053) and Sherbourne (*p* \< .001). Meanwhile, travelers heading west from Bay station rated the subjective-distance of St. George (*p* \< .001) and Spadina (*p* \< .001) as closer than did eastbound participants.

#### Reference

Maglio, S. J., & Polman, E. (2014). Spatial orientation shrinks and expands psychological distance.

Psychological Science, 25, 1345-1352.
