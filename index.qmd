...
$\epsilon = \text{error term (unexplained variation in academic performance)}$
...
By fitting this model, we can estimate the mean job satisfaction score for each department, and compare them to see if there are any significant differences. For example, we might find that the estimated mean job satisfaction score for Department 1 is 75, for Department 2 is 80, and for Department 3 is 70. This would suggest that there are significant differences in job satisfaction scores across departments, and we can further investigate why this might be the case.
...
Behmer and Crump (2017) conducted a study with 38 participants to investigate the effect of different types of words on their typing performance. Here, the factor variable ("typing material"), had three levels: Random (random 5 letter strings), Normal (5 letter English words), and Bigrams (5 letter non-words similar to real words). The goal here was to determine if reaction time (RT) varied depending on the type of word being typed.
...
Load in the data (download from the Google dive link)
...
To do that, lets work with this data ( the mean we just calculated). See the link here.
